...
// Generate regional forecast by postcode prefix
router.post('/regional', (req, res) => {
  try {
    const db = getDb()
    const { weeks = 80, coop_ids = [], flock_ids = [] } = req.body
    const numWeeks = Math.min(parseInt(weeks) || 80, 160)

    if (!coop_ids || coop_ids.length === 0) {
      return res.status(400).json({ error: 'No barns selected for region' })
    }
    ...
    // Get historical data (last 90 days) for selected coops
    const ninetyDaysAgo = new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
    const historyQuery = `
      SELECT date, SUM(egg_count) as total_eggs
      FROM egg_production
      WHERE date >= ? AND coop_id IN (${coop_ids.map(() => '?').join(',')})
      GROUP BY date ORDER BY date ASC
    `
    const historyParams = [ninetyDaysAgo, ...coop_ids]
    const history = db.prepare(historyQuery).all(...historyParams)

    // Get active flocks for selected coops
    const flocksQuery = `SELECT * FROM flocks WHERE status = ? AND coop_id IN (${coop_ids.map(() => '?').join(',')})`
    const flockParams = ['active', ...coop_ids]
    ...
    const flockHenCounts = {}
    const flockCumulativeLoss = {}
    ...
    for (let w = 1; w <= numWeeks; w++) {
      ...
      const confidenceFactor = 0.1 + (w / numWeeks) * 0.15
    ...
    res.status(500).json({ error: 'Failed to generate regional forecast' })
  }
})
